"use client";
import { useGLTF } from "@react-three/drei";
import { useFrame } from "@react-three/fiber";
import { useRef } from "react";

export default function Model(props) {
  const { nodes, materials } = useGLTF("/models/wizard-transformed.glb");

  //useRef permette di creare un riferimento a un elemento del DOM o a un componente. In questo caso, modelRef viene utilizzato per creare un riferimento al modello 3D. Questo permette di manipolare le proprietà del modello 3D, come la rotazione, la posizione e la scala.
  const modelRef = useRef();

  useFrame((state, delta, xrFrame) => {
    /* 
    modelRef.current.position.y serve per modificare la posizione Y del modello 3D
    -1.5: per centrare il modello 3D
     Math.sin(state.clock.elapsedTime): restituisce un valore compreso tra -1 e 1 utilizzando state.clock.elapsedTime. Moltiplicando il risultato di Math.sin() per 0.15, limitiamo l'oscillazione a un range di +/- 0.15 unità sull'asse Y.
    */
    modelRef.current.position.y =
      -1.5 + Math.sin(state.clock.elapsedTime) * 0.15;
  });

  return (
    <group
      {...props}
      dispose={null}
      //ref={modelRef} per indicare che questo sara il modello 3D da manipolare
      ref={modelRef}
      position={[0, -1.5, 0]}
      scale={[0.06, 0.06, 0.06]}
      rotation={[0.25, 0, 0]}
    >
      <mesh
        name="Evil_Hathattty_mesh_Evil_HatBLN_Hat_0"
        castShadow
        receiveShadow
        geometry={nodes.Evil_Hathattty_mesh_Evil_HatBLN_Hat_0.geometry}
        material={materials.Evil_HatBLN_Hat}
        position={[-1.057, 0, 0]}
        rotation={[0, 0.224, 0]}
        scale={0.832}
      />
      <mesh
        name="WandpCylinder1_Wandq_0"
        castShadow
        receiveShadow
        geometry={nodes.WandpCylinder1_Wandq_0.geometry}
        material={materials.PaletteMaterial001}
        position={[-1.057, 0, 0]}
        rotation={[0, 0.224, 0]}
        scale={0.832}
      />
      <mesh
        name="R_shoe_tongue_mesh_BLN_Shoe_tongue_0"
        castShadow
        receiveShadow
        geometry={nodes.R_shoe_tongue_mesh_BLN_Shoe_tongue_0.geometry}
        material={materials.BLN_Shoe_tongue}
        position={[-1.057, 0, 0]}
        rotation={[0, 0.224, 0]}
        scale={0.832}
      />
      <mesh
        name="Chest_piece_mesh_BLN_chest_piece_0"
        castShadow
        receiveShadow
        geometry={nodes.Chest_piece_mesh_BLN_chest_piece_0.geometry}
        material={materials.BLN_chest_piece}
        position={[-1.057, 0, 0]}
        rotation={[0, 0.224, 0]}
        scale={0.832}
      />
      <mesh
        name="Right_Eyeball_Mesh_Lam_eyeball_0"
        castShadow
        receiveShadow
        geometry={nodes.Right_Eyeball_Mesh_Lam_eyeball_0.geometry}
        material={materials.material_0}
        position={[-1.057, 0, 0]}
        rotation={[0, 0.224, 0]}
        scale={0.832}
      />
      <mesh
        name="pants_mesh_BLN_Pants_0"
        castShadow
        receiveShadow
        geometry={nodes.pants_mesh_BLN_Pants_0.geometry}
        material={materials.BLN_Pants}
        position={[-1.057, 0, 0]}
        rotation={[0, 0.224, 0]}
        scale={0.832}
      />
      <mesh
        name="lower_coat_mesh_FK_BLN_Lower_coat_0"
        castShadow
        receiveShadow
        geometry={nodes.lower_coat_mesh_FK_BLN_Lower_coat_0.geometry}
        material={materials.BLN_Lower_coat}
        position={[-1.057, 0, 0]}
        rotation={[0, 0.224, 0]}
        scale={0.832}
      />
      <mesh
        name="R_shoe_mesh_BLN_shoes_0"
        castShadow
        receiveShadow
        geometry={nodes.R_shoe_mesh_BLN_shoes_0.geometry}
        material={materials.BLN_shoes}
        position={[-1.057, 0, 0]}
        rotation={[0, 0.224, 0]}
        scale={0.832}
      />
      <mesh
        name="Upper_coat_mesh_BLN_upper_coat_0"
        castShadow
        receiveShadow
        geometry={nodes.Upper_coat_mesh_BLN_upper_coat_0.geometry}
        material={materials.BLN_upper_coat}
        position={[-1.057, 0, 0]}
        rotation={[0, 0.224, 0]}
        scale={0.832}
      />
      <mesh
        name="upper_body_mesh_BLN_shirt_0"
        castShadow
        receiveShadow
        geometry={nodes.upper_body_mesh_BLN_shirt_0.geometry}
        material={materials.BLN_shirt}
        position={[-1.057, 0, 0]}
        rotation={[0, 0.224, 0]}
        scale={0.832}
      />
      <mesh
        name="upper_body_mesh_BLN_Shirt_collar_0"
        castShadow
        receiveShadow
        geometry={nodes.upper_body_mesh_BLN_Shirt_collar_0.geometry}
        material={materials.BLN_Shirt_collar}
        position={[-1.057, 0, 0]}
        rotation={[0, 0.224, 0]}
        scale={0.832}
      />
      <mesh
        name="Coat_collar_mesh_BLN_collar_piece_0"
        castShadow
        receiveShadow
        geometry={nodes.Coat_collar_mesh_BLN_collar_piece_0.geometry}
        material={materials.BLN_collar_piece}
        position={[-1.057, 0, 0]}
        rotation={[0, 0.224, 0]}
        scale={0.832}
      />
      <mesh
        name="BookpolySurface5_Bookblinn1_0"
        castShadow
        receiveShadow
        geometry={nodes.BookpolySurface5_Bookblinn1_0.geometry}
        material={materials.Bookblinn1}
        position={[-1.057, 0, 0]}
        rotation={[0, 0.224, 0]}
        scale={0.832}
      />
      <mesh
        name="BookBook_corner_mesh4_Booklam_book_corners_0"
        castShadow
        receiveShadow
        geometry={nodes.BookBook_corner_mesh4_Booklam_book_corners_0.geometry}
        material={materials.Booklam_book_corners}
        position={[-1.057, 0, 0]}
        rotation={[0, 0.224, 0]}
        scale={0.832}
      />
      <mesh
        name="BookpolySurface20_BookBLN_Metal_parts_0"
        castShadow
        receiveShadow
        geometry={nodes.BookpolySurface20_BookBLN_Metal_parts_0.geometry}
        material={materials.BookBLN_Metal_parts}
        position={[-1.057, 0, 0]}
        rotation={[0, 0.224, 0]}
        scale={0.832}
      />
      <mesh
        name="BookFront_glow_mesh_Booklambert8_0"
        castShadow
        receiveShadow
        geometry={nodes.BookFront_glow_mesh_Booklambert8_0.geometry}
        material={materials.PaletteMaterial002}
        position={[-1.057, 0, 0]}
        rotation={[0, 0.224, 0]}
        scale={0.832}
      />
      <mesh
        name="BookpolySurface2_BookBLN_Book_covers_0"
        castShadow
        receiveShadow
        geometry={nodes.BookpolySurface2_BookBLN_Book_covers_0.geometry}
        material={materials.BookBLN_Book_covers}
        position={[-1.057, 0, 0]}
        rotation={[0, 0.224, 0]}
        scale={0.832}
      />
      <mesh
        name="BookpCube10_Booklam_front_bits_0"
        castShadow
        receiveShadow
        geometry={nodes.BookpCube10_Booklam_front_bits_0.geometry}
        material={materials.material_2}
        position={[-1.057, 0, 0]}
        rotation={[0, 0.224, 0]}
        scale={0.832}
      />
      <mesh
        name="BookpolySurface28_Booklam_back_bits_0"
        castShadow
        receiveShadow
        geometry={nodes.BookpolySurface28_Booklam_back_bits_0.geometry}
        material={materials.Booklam_back_bits}
        position={[-1.057, 0, 0]}
        rotation={[0, 0.224, 0]}
        scale={0.832}
      />
      <mesh
        name="Evil_HatpPlane1_Evil_HatsolidShader_0"
        castShadow
        receiveShadow
        geometry={nodes.Evil_HatpPlane1_Evil_HatsolidShader_0.geometry}
        material={materials.PaletteMaterial003}
        position={[-1.057, 0, 0]}
        rotation={[0, 0.224, 0]}
        scale={0.832}
      />
    </group>
  );
}

useGLTF.preload("/models/wizard-transformed.glb");
